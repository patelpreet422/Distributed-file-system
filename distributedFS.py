...
class Memory(LoggingMixIn, Operations):
    ...
    def __init__(self):
        ...
        self.m_server = xmlrpclib.ServerProxy(
            "http://localhost:"+str(int(argv[2])), allow_none=True)
        self.fd = 0
        now = time()
...
# methods for data server
    ...
    def read(self, path, size, offset, fh):
        # return string starting from offset
        return self.d_manager.read(path, size, offset, fh)
...
if __name__ == '__main__':
    if len(argv) > 8:
        print(
            'usage: %s <mountpoint> <meta_server port> <data_server ports>' % argv[0])
