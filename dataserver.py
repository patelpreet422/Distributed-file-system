...
class Data:
        ...
        if not isdir:
            for key in self.data.keys():
                ...
                if key_path == old:
                    ...
                    tup = (new, key_blkn)
                    data_store[pickle.dumps(tup)] = data_store.pop(
                        pickle.dumps(key))
        ...
        for key in self.data.keys():
            ...
            if key_path == path:
                if(key_blk > block):
                    ...
                    # update data_store
                    data_store[pickle.dumps(key)] = [new_str[:offset_pos], self.checksum(
                        self.data[key][0]), self.data[key][2]]
    ...
    def write(self, path, data, checksum, offset, copyn, fh):
        ...
        self.data[(path, block)][1] = self.checksum(
            self.data[(path, block)][0])
        self.data[(path, block)][2] = copyn
        # update data_store
        tup = (path, block)
        data_store[pickle.dumps(tup)] = [
            data_string + data, self.checksum(self.data[(path, block)][0]), copyn]
...
def serve(port):
    file_server = SimpleXMLRPCServer.SimpleXMLRPCServer(
        ('', port), allow_none=True)
