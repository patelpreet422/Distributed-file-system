...
class Manager:
    def __init__(self, ports, serv_count):
        ...
        for i in range(self.server_count):
            self.d_server.append(xmlrpclib.ServerProxy(
                "http://localhost:"+str(self.server_ports[i]), allow_none=True))
        ...
        if path in self.hash_table:
            ...
            for i in range(self.server_count):
                try:
                    ...
                    while True:
                        print("Data Server", i,
                              "is currently down. Cannot proceed with operation")
                        try:
    ...
    def unlink(self, path):
        for i in range(self.server_count):
            try:
                ...
                while True:
                    print(
                        "Data Server", i, "is currently down. Cannot proceed with truncate operation")
                    try:
                        absent = self.d_server[i].check_status()
                        # Does not build data if absent=0
                        if absent == 1:
            ...
            for key in data_dict.keys():
                ...
                if (checksum_data != self.checksum(block_data)):
                    ...
                    while True:
                        ...
                        if (k != i):
                            try:
                                ret_obj = self.d_server[k].getdata(
                                    key_path, key_blk)
                                ...
                                if (ret_data != []):
                                    ...
                                    if (checksum_data == self.checksum(blkdat)):
                                        ...
                                        self.d_server[i].putdata(
                                            key_path, key_blk, blkdat, checksum_data, copyn)
                                        break
    ...
    def rename(self, old, new, isdir):
        for i in range(self.server_count):
            try:
                ...
                while True:
                    print(
                        "Data Server", i, "is currently down. Cannot proceed with rename operation")
                    try:
                        absent = self.d_server[i].check_status()
                        # Does not build data if absent=0
                        if absent == 1:
    ...
    def write(self, path, data, offset, fh):
        ...
        try:
            ...
            self.d_server[sel_serv].write(
                path, store_data, checksum_val, offset_number, 0, fh)
        except:
            while True:
                print("Data Server", sel_serv,
                      "is currently down. Cannot proceed with write operation")
                try:
                    # store block in assigned server
        ...
        try:
            ...
            self.d_server[sel_redndt1].write(
                path, store_data, checksum_val, offset_number, 1, fh)
        except:
            while True:
                print("Data Server", sel_redndt1,
                      "is currently down. Cannot proceed with write operation")
                try:
                    # store 1st redundant copy
        ...
        try:
            ...
            self.d_server[sel_redndt2].write(
                path, store_data, checksum_val, offset_number, 2, fh)
        except:
            while True:
                print("Data Server", sel_redndt2,
                      "is currently down. Cannot proceed with write operation")
                try:
                    # store 2nd redundant copy
    ...
    def symlink(self, target, source):
        ...
        while(blk_num <= last_blk):
            ...
            try:
                self.d_server[sel_serv].symlink(
                    target, blk_num, store_data, checksum_val, 0)
            except:
                while True:
                    print("Data Server", sel_serv,
                          "is currently down. Cannot proceed with creating symlink")
                    try:
                        # store block in assigned server
                        absent = self.d_server[sel_serv].check_status()
                        # Does not build data if absent=0
            ...
            try:
                # store 1st redundant copy
                self.d_server[sel_redndt1].symlink(
                    target, blk_num, store_data, checksum_val, 1)
            except:
                while True:
                    print("Data Server", sel_redndt1,
                          "is currently down. Cannot proceed with creating symlink")
            ...
            try:
                # store 2nd redundant copy
                self.d_server[sel_redndt2].symlink(
                    target, blk_num, store_data, checksum_val, 2)
            except:
                while True:
                    print("Data Server", sel_redndt2,
                          "is currently down. Cannot proceed with creating symlink")
